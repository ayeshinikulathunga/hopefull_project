        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($request->RequestType === 'Monetary') {
                ...
                if (empty($data['amount_err']) && empty($data['payment_method_err'])) {
                    ...
                    if ($this->donationModel->addMonetaryDonation($donationData)) {
                        flash('donation_success', 'Your donation has been processed successfully!');
                        redirect('donations');
                    } else {
                ...
                if (empty($data['quantity_err'])) {
                    ...
                    if ($this->donationModel->addNonMonetaryDonation($donationData)) {
                        flash('donation_success', 'Your donation has been recorded. Thank you!');
                        redirect('donations');
                    } else {
                        flash('donation_error', 'Something went wrong. Please try again.', 'alert alert-danger');
                    }
            ...
            $this->view('donations/donate', $data);
        } else {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($request->RequestType === 'Monetary') {
                ...
                if (empty($data['amount_err']) && empty($data['payment_method_err'])) {
                    // Create donation record
                    $donationData = [
                        'donorId' => $_SESSION['donor_id'], // Changed from user_id to donor_id
                        'requestId' => $requestId,
                        'donationType' => 'Monetary', // Added donation type
                        'amount' => $data['amount'],
                        'quantity' => null, // Added null for non-applicable field
                        'isAnonymous' => isset($_POST['anonymous']) ? 1 : 0, // Added anonymous preference
                        'notes' => $data['notes']
                    ];
                ...
                if (empty($data['quantity_err'])) {
                    ...
                    $donationData = [
                        'donorId' => $_SESSION['donor_id'], // Changed from user_id to donor_id
                        'requestId' => $requestId,
                        'donationType' => 'NonMonetary', // Added donation type
                        'amount' => null, // Added null for non-applicable field
                        'quantity' => $data['quantity'],
                        'isAnonymous' => isset($_POST['anonymous']) ? 1 : 0, // Added anonymous preference
