        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if(isset($_FILES['product_image']) && $_FILES['product_image']['error'] === 0) {
                ...
                if(move_uploaded_file($_FILES['product_image']['tmp_name'], $upload_file)) {
                    ...
                    if(!empty($product->ProductImage) && file_exists($upload_dir . $product->ProductImage)) {
                        unlink($upload_dir . $product->ProductImage);
                    }
                }
            }
   ...
   /* public function editProduct($id) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($this->sellerModel->deleteProduct($id, $seller->SellerID)) {
                ...
                if(!empty($product->ProductImage)) {
                    $upload_dir = UPLOADS_PATH . '/products/';
                    if(file_exists($upload_dir . $product->ProductImage)) {
                        unlink($upload_dir . $product->ProductImage);
                    }
                }
    ...
    /*public function deleteProduct($id) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($this->sellerModel->deleteProduct($id, $seller->SellerID)) {
                // Delete all product images with pattern ProductID.*
                $uploadDir = ROOT_PATH . '/public/uploads/products/';
                $validExtensions = ['jpg', 'jpeg', 'png', 'gif'];
                
                // Try to delete any image with the ProductID.* pattern
                foreach($validExtensions as $ext) {
                    $imagePath = $uploadDir . $id . '.' . $ext;
                    if(file_exists($imagePath)) {
                        unlink($imagePath);
                ...
                // Also check for original image format if it exists
                if(!empty($product->ProductImage)) {
            ...
            if(in_array(strtolower($fileExt), $validExtensions)) {
                ...
                if($_FILES['product_image']['size'] <= 2 * 1024 * 1024) {
                    ...
                    if(move_uploaded_file($_FILES['product_image']['tmp_name'], $uploadFile)) {
                        ...
                        $uploadError = "Failed to upload image. Please try again.";
                        error_log("Failed to move uploaded file. Source: " . $_FILES['product_image']['tmp_name'] . " Destination: " . $uploadFile);
                    }
                } else {
                    $uploadError = "File is too large. Maximum size is 2MB.";
                }
